...
function UserWithDB() {
  ...
  const fetchData = async () => {
    try {
      const result = await axios.get(
        `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/users_with_api`,
        {
          headers: {
  ...
  const deleteData = async (id) => {
    try {
      const { error } = await supabase.from("users_with_api").delete().eq("id", id);
      if (error) {
        throw error;
      }
      setUpdate(update + 1);
  ...
  const updateData = async (payload) => {
    ...
    try {
      const data = await axios.patch(
        ...
        }/rest/v1/users_with_api?id=eq.${updateForm}`,
        payload,
        {
          headers: {
            "Content-Type": "application/json",
